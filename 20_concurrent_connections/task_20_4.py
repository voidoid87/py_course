...
def send_command(device, command):
	...
	with ConnectHandler(**device) as s:
		...
		if type(command) == list:
			for c in command:
				result = result + s.find_prompt().strip() + s.send_command(c, strip_command = False) + '\n'
...
def send_config_command(device, command):
	...
	with ConnectHandler(**device) as s:
		s.enable()
		result = result + s.find_prompt().strip() + s.send_config_set(command, strip_command = False) + '\n'
		logging.info('<-- Received from {}'.format(device['ip']))
	return result
...
def send_commands_to_devices(devices, filename, show = None, config = None, limit = 3):
	...
	with ThreadPoolExecutor(max_workers = limit) as executor:
		for device in devices:
			if show:
				future_list.append(executor.submit(send_command, device, show))
			elif config:
				future_list.append(executor.submit(send_config_command, device, config))
...
if __name__ == '__main__':
	...
	send_commands_to_devices(devices, filename = 'output_20_4.txt', config = config)
